...
namespace nilnul.fs.folders_._isolated.item.div_.unjoint_.nonneglect
{
	public class Prjs
	{
		...
		public IEnumerable<string> seq_ofAddress(
			...
			switch (category)
			{
				case fs.folder.categorize_.git_._plainWorkRepo.Ret.Plain:
					...
					return new div_.unjoint_.nonneglect_.giT_.plain.Prjs(io).seq_ofAddress(folder);
					break;
				case fs.folder.categorize_.git_._plainWorkRepo.Ret.Work:
					return new div_.unjoint_.nonneglect_.giT_.work.Prjs(io).seq_ofAddress(folder);
